...
| 81 | Сверил таблицу трудозатрат | 0.5 |
| 82 | Удалил AGENTS.md из истории | 0.5 |
| 83 | Добавил логирование MQTT-пакетов | 2.5 |
...
- Закрепил в RFC и агентских инструкциях правило использовать нативный PHPUnit
  для unit-тестов и проверил, что исходники `core`/`bus` не используют Mockery.
- Расширил архитектурный RFC: описал причины выбора нативного PHPUnit для
  unit-тестов и будущую декомпозицию `core` на микросервисы через DDD bounded
  contexts, CQRS-контракты и hexagonal ports/adapters.
...
- Сверил таблицу трудозатрат с `git log --reverse`, подтвердил совпадение
  порядка и текстов коммитов, исправил арифметический итог.
- Удалил `AGENTS.md` из отслеживаемых файлов, оставил его в `.gitignore` и
  переписал историю, чтобы файл не присутствовал в прошлых коммитах.
- Добавил в `bus` структурированное JSON-логирование MQTT-пакетов, полученных
  от Mosquitto, и дашборд Grafana `MQTT Bus Packets` для просмотра Loki-логов и
  Prometheus-throughput пайплайна.
- Восстановил стендовый MQTT pipeline: добавил внутренний listener Mosquitto
  `1883` для bus и учел ответ phpredis `BUSYGROUP` при повторном создании Redis
